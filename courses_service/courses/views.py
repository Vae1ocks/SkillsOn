...
class CourseOverviewList(GenericAPIView):
            ...
            200: OpenApiResponse(
                response=inline_serializer(
                    ...
                    fields={
                        ...
                        "some_other_category_title": serializers.CourseSerializer(
                            many=True
                        ),
                    },
                )
    ...
    def get(self, request, *args, **kwargs):
        ...
        most_popular = (
            ...
            .filter(id__in=most_liked.values("id"))
        ...
        if self.request.user.is_authenticated:
            ...
            if (
            ...
            ):  # [{'id': 5, 'title': 'title}, {'id': 6, 'title': 'titttle'}]
                ...
                for category in categories_liked:
                    rec_queryset = (
                        Course.published.filter(category=category)
                        .exclude(id__in=most_popular_ids)
                        .order_by("-rating")[:8]
                    )
                    data = serializers.CourseSerializer(rec_queryset, many=True).data
                    result[category.title] = data
        categories = Category.objects.exclude(id__in=categories_liked).only(
            "id", "title"
        )
...
class CourseViewSet(ModelViewSet):
    def get_queryset(self):
        if self.action == "list":
            ...
            if category:
                ...
                ).filter(lower_title__in=cat_titles)
            ...
            if allowable_price:
                try:
                    ...
                    raise ParseError(
                        'Параметр "price" должен быть в ' 'формате "min;max"'
                    )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if (not course.moderated or course.draft) and (
            course.author != request.user.id
        ):
            return Response(
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="title",
                description="Параметр для фильтрации по " "названию товаров",
                required=False,
                type=OpenApiTypes.STR,
            ),
            OpenApiParameter(
                name="allowable_price",
                description="Параметр для фильтрации по " "цене в формате min;max",
                required=False,
    ...
    def perform_destroy(self, instance):
        if len(instance.students) != 0:
            return Response(
                {"detail": "more than 0 students"}, status=status.HTTP_403_FORBIDDEN
            )
        instance.delete()
...
class UserCourseListView(ListAPIView):
    ...
    @extend_schema(
        description='Для просмотра купленных и созданных курсов. "is_owner" - булево значение, '
...
class ValidateUserPreferencesView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for category in categories:
            categories_data.append({"id": category.id, "title": category.title})
