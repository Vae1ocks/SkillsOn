...
class YooKassaPayoutView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            202: inline_serializer(
                name="PayoutCreated", fields={"detail": serializers.CharField}
            )
        },
    )
    def post(self, request, *args, **kwargs):
        ...
        if user.balance < value:
            return Response(
                {"detail": "Недостаточно средств"}, status=status.HTTP_400_BAD_REQUEST
            )
        if not payout_token or not value:
            return Response(
                {"detail": "Не переданы данные для платежа"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        idempotence_key = str(uuid.uuid4())

        user_payout = UserPayout.objects.create(
            user=user, value=value, status="pending"
        )
        ...
        return Response(
            {"detail": "Выплата успешно создана"}, status=status.HTTP_202_ACCEPTED
        )
