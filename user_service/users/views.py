...
class UserPersonalInfoUpdateView(APIView):
    ...
    @extend_schema(description="Для смены персональных данных")
...
class ConfirmationOldEmailView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"detail": "Требуется ввод кода"}, status=status.HTTP_400_BAD_REQUEST
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class EmailUpdateSetNewEmailView(APIView):
    ...
    @extend_schema(description="Третий этап смены почты: ввод новой электронной почты.")
    def post(self, request, *args, **kwargs):
...
class EmailUpdateFinish(APIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = serializers.EmailWithConfirmationCodeSerializer(data=request.data)
        if serializer.is_valid():
            exp_code = request.session.get("confirmation_code")
            if exp_code:
                entered_code = serializer.validated_data["code"]
                if exp_code == entered_code:
                    ...
                    current_app.send_task(
                        ...
                        kwargs={"old_user_email": user.email, "new_user_email": email},
        ...
        if name:
            ...
            if len(name) == 2:
                ...
                queryset = get_user_model().objects.filter(
                    Q(
                        first_name__icontains=first_value,
                        last_name__icontains=second_value,
                    )
                    | Q(
                        first_name__icontains=second_value,
